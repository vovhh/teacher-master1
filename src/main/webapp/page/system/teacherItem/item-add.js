layui.use(['form','layer','upload','laydate','transfer','util'],function(){
    ...
    //执行一个laydate实例
    laydate.render({elem: '#createTime',trigger: 'click' , type: 'year', done: function(value, date, endDate){//控件选择完毕后的回调
            //当用户选中立项时，起始时间变为当年1.1.结束时间变为12.31
            $(".startTime").val(value+"-01-01");
            $(".endTime").val(value+"-12-31");
        }});
    laydate.render({elem: '#startTime',trigger: 'click' , type: 'date', done: function(value, date, endDate){}});
    laydate.render({elem: '#endTime',trigger: 'click' , type: 'date', done: function(value, date, endDate){}});
    ...
    form.on("submit(addUser)",function(data){
        ...
        // 实际使用时的提交信息
        $.post(updateFlag==='0'?"/teacher/biz/teacherItem_save.action":"/teacher/biz/teacherItem_update.action",{
