layui.config({
    ...
    var tableIns = table.render({
        ...
        url: '/teacher/biz/teacherItem_findByDept.action?deptId='+deptId,//数据接口
        page : true,//开启分页
    ...
    table.on('tool(infoList)', function (obj) {
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get("/teacher/biz/teacherItem_delete.action",{
    ...
    $(".delBtn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("/teacher/biz/teacherItem_deleteBatch.action",{
    ...
    $(".exportBtn").click(function(){
        ...
        if(data.length > 0) {
            ...
            $.ajax({
                url: '/teacher/biz/teacherItem_findByExport.action',
                ...
                success: function (res) {
                    if (res.code===0){
                        ...
                        data.unshift({itemId: '编号', itemName: '项目名称', itemPerson: '主持人', itemType: '项目类型',
                            itemCategory: '项目类别', createTime: '立项时间', itemMoney: '项目经费', itemLevel: '项目级别',
                            memberName: '成员', contractNumber: '合同编号', startTime: '起始时间', endTime: '结束时间'});
                        excel.exportExcel({
                            sheet1: data
                        }, '部分教师项目.xlsx', 'xlsx');
                    }else {
        ...
        var index = layui.layer.open({
            ...
            success : function(layero, index){
                ...
                if(edit){
                    ...
                    body.find(".personHide").val(edit.itemPerson.teacherCode);//使用中间变量记录负责人
    ...
    $.post("/teacher/biz/dept_findSubordinate.action?deptId="+deptId, function (data) {//--所属教研室
        ...
        $.each(data.data, function (index, item) {
            $('#unitSelect').append(new Option(item.deptName, "2,"+item.deptId));
        });
        //重新渲染select
        form.render('select');
    });
    $.post("/teacher/biz/teacherItemCategory_findAll.action", function (data) {//--教师项目类别
        $('#itemCategorySelect').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#itemCategorySelect').append(new Option(item.title, "4,"+item.id));
        });
        //重新渲染select
        form.render('select');
    });
    $.post("/teacher/biz/teacherItemLevel_findAll.action", function (data) {//--教师项目级别
        $('#itemLevelSelect').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#itemLevelSelect').append(new Option(item.title, "5,"+item.id));
        });
        //重新渲染select
        form.render('select');
    });
    // $.post("/teacher/biz/teacher_findAllByDept.action?deptId="+deptId, function (data) {//--主持人
    //     $('#personSelect').append(new Option("请选择", ""));
    //     $.each(data.data, function (index, item) {
    //         $('#personSelect').append(new Option(item.teacherName, "6,"+item.teacherId));
    //     });
    //     //重新渲染select
    //     form.render('select');
    // });
    ...
    form.on("select(itemTypeSelect)", function (data) {
        reloadTable(data)
    });
    form.on("select(itemCategorySelect)", function (data) {
        reloadTable(data)
    });
    form.on("select(itemLevelSelect)", function (data) {
        reloadTable(data)
    });
