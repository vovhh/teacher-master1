layui.use(['form', 'layer', 'table', 'laytpl', 'util', 'laydate'], function () {
    ...
    var tableIns = table.render({
        ...
        url: '/teacher/biz/paper_findByDept.action?deptId='+deptId,//数据接口
        page: true,//开启分页
    ...
    table.on('tool(infoList)', function (obj) {
        ...
        if (layEvent === 'edit') { //编辑
            ...
            layer.confirm('确定删除此记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get("/teacher/biz/paper_delete.action", {
    ...
    $(".delBtn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("/teacher/biz/paper_deleteBatch.action",{
        ...
        var index = layui.layer.open({
            ...
            success : function(layero, index){
                ...
                if(edit){
                    ...
                    body.find(".personHide").val(edit.teacher.teacherCode);//使用中间变量记录作者
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/teacher/biz/paperAnnex_findByPaperId.action',//数据接口
                            ...
                            if (data.code === 0) {
                                ...
                                $.each(data.data, function (index, item) {
                                    ...
                                    oldtr.find('.demo-delete').on('click', function(){
                                        $.post("/teacher/biz/paperAnnex_delete.action", {
    ...
    laydate.render({elem: '#queryTime',trigger: 'click' , type: 'year', done: function(value, date, endDate){//控件选择完毕后的回调
        // $('#paperTypeSelect').val(null);$('#paperGradeSelect').val(null);$('#unitSelect').val(null);$('#authorSelect').val(null);
        table.reload('infoListTable', {
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: {key: "1,"+value}}, 'data');
        }});
    $.post("/teacher/biz/paperGrade_findAll.action", function (data) {//--论文等级
        $('#paperGradeSelect').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#paperGradeSelect').append(new Option(item.title, "2,"+item.id));
        });
        //重新渲染select
        form.render('select');
    });
    $.post("/teacher/biz/dept_findSubordinate.action?deptId="+deptId, function (data) {//--所属教研室
        $('#unitSelect').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#unitSelect').append(new Option(item.deptName, "4,"+item.deptId));
        });
        //重新渲染select
        form.render('select');
    });
    $.post("/teacher/biz/teacher_findAllByDept.action?deptId="+deptId, function (data) {//--主持人
        $('#authorSelect').append(new Option("请选择", ""));
        $.each(data.data, function (index, item) {
            $('#authorSelect').append(new Option(item.teacherName, "5,"+item.teacherId));
        });
        //重新渲染select
        form.render('select');
    });
    ...
    form.on("select(paperGrade)", function (data) {
        // $('#queryTime').val(null);$('#paperTypeSelect').val(null);$('#unitSelect').val(null);$('#authorSelect').val(null);
        reloadTable(data)
    });
    form.on("select(paperType)", function (data) {
        // $('#queryTime').val(null);$('#paperGradeSelect').val(null);$('#unitSelect').val(null);$('#authorSelect').val(null);
        reloadTable(data)
    });
    form.on("select(unitId)", function (data) {
        // $('#queryTime').val(null);$('#paperGradeSelect').val(null);$('#paperTypeSelect').val(null);$('#authorSelect').val(null);
        reloadTable(data)
    });
    form.on("select(authorId)", function (data) {
        // $('#queryTime').val(null);$('#paperGradeSelect').val(null);$('#paperTypeSelect').val(null);$('#unitSelect').val(null);
        reloadTable(data)
    });
