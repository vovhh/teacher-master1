layui.use(['form', 'layer', 'table', 'laytpl', 'util', 'laydate'], function () {
    ...
    form.on("select(paperGrade)", function (data) {$('#queryTime').val('');$('#paperType').val('');reloadTable(data)});
    form.on("select(paperType)", function (data) {$('#queryTime').val('');$('#paperGrade').val('');reloadTable(data)});
