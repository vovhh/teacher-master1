layui.use(['element', 'form', 'table', 'layer', 'laytpl', 'util'], function () {
    ...
    var thisTeacherId = $('.thisTeacherId').val();
    //论文信息列表
    var paperInfoList = table.render({
        elem: '#paperInfoList',//数据表格id
        url: '/teacher/biz/paper_findByAuthor.action?teacherId=' + thisTeacherId,//数据接口
        page: true,//开启分页
        ...
        cols: [[//表头
            ...
            {field: 'publishTime', title: '发表时间', minWidth: 100, align: 'center', templet: function (d) {
    ...
    var matchInfoList = table.render({
        ...
        url: '/teacher/biz/match_findByTeacher.action?teacherId=' + thisTeacherId,//数据接口
        page : true,//开启分页
        height : "full-125",//容器高度
        limits : [10,15,20,25],
        limit : 20,
        ...
        cols : [[//表头
            ...
            {field: 'deptName', title: '所在部门', minWidth:100, align:'center', templet:function(d){
                    return d.teacher.deptName;
                }}
        ]]
    });

    //学生竞赛项目信息列表
    var stuCompetitionList = table.render({
        elem: '#stuCompetitionList',//数据表格id
        url: '/teacher/biz/competition_findByTeacher.action?matchAttribute=1&teacherId='+thisTeacherId,//数据接口
        page : true,//开启分页
        height : "full-125",//容器高度
        limits : [10,15,20,25],
        limit : 20,
        id : "stuCompetitionListTable",//给它一个id，用于下面更新表单内容
        cols : [[//表头
    ...
    //教师竞赛项目信息列表
    var teaCompetitionList = table.render({
        elem: '#teaCompetitionList',//数据表格id
        url: '/teacher/biz/competition_findByTeacher.action?matchAttribute=2&teacherId='+thisTeacherId,//数据接口
        page : true,//开启分页
        height : "full-125",//容器高度
        limits : [10,15,20,25],
        limit : 20,
        id : "teaCompetitionListTable",//给它一个id，用于下面更新表单内容
        cols : [[//表头
            // {field: 'prizeImg', title: '获奖证书', minWidth: 100, align: 'center', templet: function (d) {
            //         return '<img layer-src="'+d.prizeImg+'" src="'+d.prizeImg+'"/>';
            //     }},
    ...
    var teacherItemInfoList = table.render({
        ...
        url: '/teacher/biz/teacherItem_findByPerson.action?itemPersonId='+thisTeacherId,//数据接口
        page : true,//开启分页
        height : "full-125",//容器高度
        limits : [10,15,20,25],
        limit : 20,
        ...
        cols : [[//表头
            ...
            {field: 'memberName', title: '项目成员', minWidth:100, align:'center'},
    ...
    //个人荣誉列表
    var teacherHonorInfoList = table.render({
        elem: '#teacherHonorInfoList',//数据表格id
        url: '/teacher/biz/teacherHonor_findByPerson.action?teacherId='+thisTeacherId,//数据接口
        page : true,//开启分页
        height : "full-125",//容器高度
        limits : [10,15,20,25],
        limit : 20,
        id : "teacherHonorInfoListTable",//给它一个id，用于下面更新表单内容
        cols : [[//表头
            {type: "checkbox", fixed:"left", width:50},
    ...
    //主编教材列表
    var teachingMaterialInfoList = table.render({
        elem: '#teachingMaterialInfoList',//数据表格id
        url: '/teacher/biz/teachingMaterial_findByEditor.action?teacherId='+thisTeacherId,//数据接口
        page : true,//开启分页
        height : "full-125",//容器高度
        limits : [10,15,20,25],
        limit : 20,
        id : "teachingMaterialInfoListTable",//给它一个id，用于下面更新表单内容
        cols : [[//表头
            {type: "checkbox", fixed:"left", width:50},
    ...
    //教学成果列表
    var teachingResultInfoList = table.render({
        elem: '#teachingResultInfoList',//数据表格id
        url: '/teacher/biz/teachingResult_findByPerson.action?teacherId='+thisTeacherId,//数据接口
        page : true,//开启分页
        height : "full-125",//容器高度
        limits : [10,15,20,25],
        limit : 20,
        id : "teachingResultInfoListTable",//给它一个id，用于下面更新表单内容
        cols : [[//表头
            {type: "checkbox", fixed:"left", width:50},
    ...
    $(document).on('click','#imgPreview',function(){
        ...
        layer.open({
            ...
            content: '<div style="text-align:center"><img src="' + $('#imgPreview').attr('src') + '" /></div>'
        });
    });
