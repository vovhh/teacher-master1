layui.use(['form','layer','table','laytpl','util'],function(){
    ...
    var tableIns = table.render({
        ...
        url: '/teacher/biz/teacher_findByDept.action?deptId='+deptId,//数据接口
        cellMinWidth : 95,
        page: true,//开启分页
    ...
    $(".delBtn").click(function () {
        ...
        if (data.length > 0) {
            ...
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("/teacher/biz/teacher_deleteBatch.action", {
    ...
    table.on('tool(infoList)', function (obj) {
        ...
        if (layEvent === 'edit') { //编辑
            ...
            layer.confirm('确定删除此记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get("/teacher/biz/teacher_delete.action", {
        ...
        var index = layui.layer.open({
            ...
            success: function (layero, index) {
                ...
                if (edit) {
                    ...
                    body.find(".roleHide").val(edit.roleId);  //使用中间变量保存角色id
