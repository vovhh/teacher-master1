layui.use(['form','layer','upload','laydate'],function(){
    ...
    $.post("/teacher/role_findByOtherRole.action", function (data) {
        $.each(data.data, function (index, item) {
            // $('#roleId').append('<input type="checkbox" name="roleIds" value="'+item.roleId+'" lay-skin="primary" title="'+item.roleName+'"/>');
            // $('#roleId').append('<input type="radio" name="roleIds" value="'+item.roleId+'" title="'+item.roleName+'"/>');
            $('#roleId').append(new Option(item.roleName, item.roleId));
        });
        if (updateFlag==='1') {//更新的时候使用中间变量让其默认选中
            $('#roleId').val($('.roleHide').val()).prop("disabled", true);  //角色-无法修改
        }
        // form.render('radio');
        form.render('select');
    });
    ...
    var uploadInst = upload.render({
        elem: '#imgUpload',
        // url: '/teacher/upload_uploadTeacherImg.action',
