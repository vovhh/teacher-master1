layui.config({
    ...
    $(".delBtn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("/teacher/biz/teachingResult_deleteBatch.action",{
    ...
    function addUser(edit){
        var index = layui.layer.open({
            title : "添加",
            type : 2,
            content : "result-add.html",
            maxmin: true,
            btn: ['确定', '取消'],
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                if(edit){
                    body.find(".Id").val(edit.resultId);
                    body.find(".resultName").val(edit.resultName);
                    body.find(".resultTime").val(util.toDateString(edit.resultTime, "yyyy"));
                    body.find(".personHide").val(edit.itemPerson.teacherCode);//使用中间变量记录--因为赋值的时候result-add.html的下拉框渲染还没开始，无法选中
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/teacher/biz/teachingResultAnnex_findByResultId.action',//数据接口
                        data: {resultId: edit.resultId},
                            ...
                            if (data.code === 0) {
                                ...
                                $.each(data.data, function (index, item) {
                                    ...
                                    oldtr.find('.demo-delete').on('click', function(){
                                        $.post("/teacher/biz/teachingResultAnnex_delete.action", {
    ...
    function detailUser(data) {
        var index = layui.layer.open({
            ...
            success: function (layero, index) {
                ...
                body.find(".memberName").val(data.memberName);//主要完成人
