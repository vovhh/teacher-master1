layui.use(['form','layer','upload','laydate','transfer','util'],function(){
    ...
    //执行一个laydate实例
    laydate.render({elem: '#resultTime',trigger: 'click' , type: 'year', done: function(value, date, endDate){}});
    ...
    form.on("submit(addUser)",function(data){
        ...
        // 实际使用时的提交信息
        $.post(updateFlag==='0'?"/teacher/biz/teachingResult_save.action":"/teacher/biz/teachingResult_update.action",{

            resultId : updateFlag==='0'?null:$(".Id").val(),
            resultName : $(".resultName").val(),
    ...
    //添加验证规则
    form.verify({
        userFile: function (value) {if (value == null || value=="") return "请上传教学成果附件！";}
    })
