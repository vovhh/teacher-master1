layui.use(['form','layer','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        ...
        url: '/teacher/biz/match_findByPage.action',//数据接口
        page : true,//开启分页
        ...
        cols : [[//表头
            ...
            {field: 'matchAttribute', title: '赛事属性', minWidth:50, align:'center', templet:function(d){
                    if (d.matchAttribute=='1') return '学生赛事';
                    if (d.matchAttribute=='2') return '教师赛事';
                }},
    ...
    table.on('tool(infoList)', function (obj) {
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get("/teacher/biz/match_delete.action",{
    ...
    $(".delBtn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.post("/teacher/biz/match_deleteBatch.action",{
        ...
        var index = layui.layer.open({
            ...
            success : function(layero, index){
                ...
                if(edit){
                    ...
                    body.find(".matchName").val(edit.matchName);
                    ...
                    $.ajax({
                        type: "POST",
                        url: '/teacher/biz/matchAnnex_findByMatchId.action',//数据接口
                            ...
                            if (data.code === 0) {
                                ...
                                $.each(data.data, function (index, item) {
                                    ...
                                    oldtr.find('.demo-delete').on('click', function(){
                                        $.post("/teacher/biz/matchAnnex_delete.action", {
    ...
    function detailUser(data) {
        var index = layui.layer.open({
            ...
            success: function (layero, index) {
                ...
                body.find(".deptName").val(data.teacher.deptName);
                var str = "";   //附件
                $.ajax({
                    type: "POST",
                    url: '/teacher/biz/matchAnnex_findByMatchId.action',//数据接口
                    ...
                    success: function (data) {
                        if (data.code === 0) {
                            $.each(data.data, function (index, item) {
                                str += '<a href="/teacher/download/download_downloadFile.action?' +
