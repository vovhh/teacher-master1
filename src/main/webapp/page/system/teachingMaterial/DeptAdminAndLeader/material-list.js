layui.config({
    ...
    var tableIns = table.render({
        ...
        url: '/teacher/biz/teachingMaterial_findByDept.action?deptId='+deptId,//数据接口
        page : true,
        height : "full-125",
        limits : [10,15,20,25],
        limit : 20,
    ...
    $.post("/teacher/biz/teacher_findAllByDept.action?deptId="+deptId, function (data) {//--主持人
        ...
        $.each(data.data, function (index, item) {
            $('#personSelect').append(new Option(item.teacherName, "2,"+item.teacherId));
        });
        //重新渲染select
        form.render('select');
    });
    $.post("/teacher/biz/dept_findSubordinate.action?deptId="+deptId, function (data) {//--所属教研室
        ...
        $.each(data.data, function (index, item) {
            $('#unitSelect').append(new Option(item.deptName, "3,"+item.deptId));
        });
        //重新渲染select
        form.render('select');
    });
    ...
    form.on("select(personSelect)", function (data) {
        reloadTable(data)
    });
