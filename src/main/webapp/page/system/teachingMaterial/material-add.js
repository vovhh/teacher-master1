layui.use([ 'form', 'layer', 'upload', 'laydate'], function () {
    ...
    // $.post("/teacher/biz/teacher_findAllByDept.action?deptId="+deptId, function (data) {//--教材主编
    ...
    //执行一个laydate实例
    laydate.render({elem: '#publishTime',trigger: 'click', type: 'year', done: function (value, date, endDate) {}});

    form.on("submit(addUser)", function (data) {
        //弹出loading
        var index = layer.msg('数据提交中，请稍候', {icon: 16, time: false, shade: 0.8});
        // 实际使用时的提交信息
        $.post(updateFlag === '0' ? "/teacher/biz/teachingMaterial_save.action" : "/teacher/biz/teachingMaterial_update.action", {

            materialId: updateFlag === '0' ? null : $(".Id").val(),//id
    ...
    // 附件上传要求--添加验证规则
    form.verify({
        userFile: function (value) {if (value == null || value=="") return "请上传教材附件！";}
    })
});