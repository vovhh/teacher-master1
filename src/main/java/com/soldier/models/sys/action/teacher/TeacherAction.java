package com.soldier.models.sys.action.teacher;
...
import com.soldier.models.sys.service.sysuser.SysUserService;
...
//    private Long deptId;
    ...
    public String save(){
        ...
        if (teacherService.findByTeacherCode(teacherEntity) == null) {

            teacherEntity.setUpdateTime(new java.sql.Date(new Date().getTime()));

            if (teacherEntity.getCanLook() == null) teacherEntity.setCanLook(1);//是否允许除系统管理员之外的角色查看，1-允许 0-不允许
            ...
            dept_result.setDeptId(teacherEntity.getDeptId());
            dept_result = deptService.findById(dept_result);
            teacherEntity.setDeptName(dept_result.getDeptName());
            ...
            r = R.error(444, "该工号：『"+teacherEntity.getTeacherCode()+"』教师已存在，请重新输入！");
        }
    ...
    public String updateRole(){
        ...
        teacherEntity_result.setRoleId(teacherEntity.getRoleId());
        teacherEntity_result.setRoleName(teacherEntity.getRoleName());
    ...
    public String findByDept(){
        ...
        //如果是系统管理员,部门为空
        PageBean byDeptTeacher = teacherService.findByDept(teacherEntity.getDeptId(), key, new PageBean<TeacherEntity>().setCurrPage(page).setPageSize(limit));
    ...
    public String findAllByDept(){
        ...
        List list = teacherService.findAllByDept(teacherEntity.getDeptId());
